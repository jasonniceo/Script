...
# 先定义「颜色 / 配置 / 变量」→ 再定义「所有子函数」（按main()内调用顺序）→ 再定义main()函数（核心执行逻辑）→ 最后main（脚本入口）。阅读时无需前后跳转，完全按 “定义→调用→执行结束” 的顺序，更符合直觉。
...
#######################################
# 函数：自动发现acme.sh中的域名（main中调用，多处使用）
#######################################
...
#######################################
# 函数：时间格式转换（show_cert_status/check_cert_expiry中调用）
#######################################
convert_cert_time() {
    local raw_time="$1"
